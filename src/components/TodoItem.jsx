...
export const TodoItem = ({ todo, onDelete, onToggleComplete, onUpdate }) => {
  ...
  const handleSave = useCallback(() => {
    if (editText.trim()) {
      onUpdate(todo.id, editDeadline, editText);
    }
    setIsEditing(false);
  ...
  return (
    ...
    justify-between p-4 gap-3 bg-white dark:bg-page-dark rounded-lg 
     shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100"
    >
      <div className="flex flex-row items-center gap-2">
        <div
          {...listeners}
          className="h-6 w-4 border-l-6 border-r-6 border-gray-300 border-dotted mx-0.5 
